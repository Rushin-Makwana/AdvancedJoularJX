...
## New Features
We have extended the original JoularJX as a part of our research work to support some additional features, including:

1) Added Capabililty to monitor total energy along with self energy for each methods.
2) Added per-thread energy monitoring for each method. (This can toggle on/off in the config.properties file)
3) Differentiated among overloaded methods with same name but different parameters. 
4) Captured last sampling thread with actual value (replace old logic of assuming a 10 seconds jump in the thread sampling). This is to avoid missing some methods that are called and finished in the last window.
