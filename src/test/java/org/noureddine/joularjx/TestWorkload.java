...
public class TestWorkload {
    ...
    public void workloadTest() throws InterruptedException {
        ...
        for (int i = 0; i < 12500; i++) {
            double a = methodA(); // numeric accumulation work
            double b = methodA(150); // small matrix-like work
            res += methodC(i);
            if (i % 500 == 0) {
                System.out.println(
                        "Iteration=" + i + " A=" + String.format("%.6f", a) + " B=" + String.format("%.6f", b));
            }
        }
    ...
    // matrix
    public static double methodA(int size) {
